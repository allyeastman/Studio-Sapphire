...
class Freshmail
{
	private $fmApiKey, $fmApiSecret, $api, $connect, $fmSettings, $reportType, $reportArray;
	...
	public function __construct()
	{
		...
		if (is_super_admin()) {
			...
			add_action('wp_ajax_preview_form', array($this, 'previewFormAjax'));
			add_action('wp_ajax_nopriv_preview_form', array($this, 'previewFormAjax'));

			add_action('wp_ajax_freshmail_change_theme', array($this, 'changeTheme'));
			add_action('wp_ajax_save_as_custom_theme', array($this, 'saveCustomTheme'));
			add_action('wp_ajax_fm_get_reports', array($this, 'getReports'));
		}
		...
		if ($this->connect === true) {
			...
			/* pop-up */
			add_action('wp_head', array($this, 'popupWpHead'), 100);
			add_action('wp_footer', array($this, 'popupWpFooter'), 100);
			...
			add_action('wp_ajax_popup_show', array($this, 'ajaxPopup'));
			...
			add_action('wp_enqueue_scripts', function (){
				...
				wp_register_script($handle, WP_FRESHMAIL_URL.'/assets/js/user.js', array('jquery', 'jquery-ui-dialog', 'jquery-ui-tooltip'));
	...
	{
		//menu
		add_menu_page(__('FreshMail', 'wp_freshmail'), __('FreshMail', 'wp_freshmail'), 'manage_options', 'freshmail', array(
			$this,
			'renderAllFormsPage'
		), WP_FRESHMAIL_URL.'/assets/images/icon.png');

		//submenu
		add_submenu_page('freshmail', __('New form', 'wp_freshmail'), __('New form', 'wp_freshmail'), 'manage_options', 'freshmail_new_form', array($this, 'renderNewFormPage'));
		...
		add_submenu_page('freshmail', __('Reports', 'wp_freshmail'), __('Reports', 'wp_freshmail'), 'manage_options', 'freshmail_reports', array($this, 'renderReportsPage'));
		add_submenu_page('freshmail', __('Connect', 'wp_freshmail'), __('Settings', 'wp_freshmail'), 'manage_options', 'freshmail_connect', array($this, 'renderConnectPage'));
	}

	public function renderAllFormsPage()
	{
		if ($this->connect !== true) {
	...
	public function renderNewFormPage()
	{
		if ($this->connect !== true) {
			$this->renderConnectPage();
			die;
	...
	private function addNewEmail($form = array(), $referer = null, $formId = null, $freshmailListId = null)
	{
		global $wpdb;
	...
	public function popupWpHead()
	{
		...
		foreach ($results as $val) {
			...
			if (($postType == 'page')) {
				if (!isset($freshmailForm['allowed_pages']) || !in_array(get_the_ID(), $freshmailForm['allowed_pages'])) {
					break;
				}
			} elseif ($postType == 'post') {
				if (!isset($freshmailForm['allowed_main_posts']) || !in_array(get_the_ID(), $freshmailForm['allowed_main_posts'])) {
					break;
				}
			} elseif ($postType == 'product') {
				if (!isset($freshmailForm['allowed_products']) || !in_array(get_the_ID(), $freshmailForm['allowed_products'])) {
					break;
				}
			} else {
				break;
			...
			if (!in_array(get_the_ID(), $allowedPages)) {
				break;
			}
	...
	public function previewFormAjax()
	{
	...
	public function freshmailSignUpForms()
	{
		if (isset($_POST['fm_form_var'])) {
			...
			if (isset($_GET['form_id']) && (int)$_GET['form_id'] != 0) {
				...
				$this->updateThemes($_POST['fm_form_var']['appearance'], $_POST['fm_form_var']['appearance']['freshmail_select_theme']);
			} else {
				$wpdb->insert(
					$wpdb->prefix.'freshmail_forms',
					array(
	...
	private function updateThemes($appearance, $key = null)
	{
		$customTheme = get_option('freshmail_custom_theme');
	...
	public function getReports()
	{
		...
		$this->showVisualizationLineChart2($columns, $this->getDataFromDb(date('Y-m-d', strtotime($params['from_y']."-".$params['from_m']."-".$params['from_d'])).' 23:59', date('Y-m-d', strtotime($params['to_y']."-".$params['to_m']."-".$params['to_d'])).' 23:59'));
		die;
	}

	private function getDataFromDb($fromDate = null, $toDate = null)
	{
		global $wpdb;
		$where = '';
			...
			if ($this->fmSettings['fm_sign_up_checkboxes']['sign_up_checkboxes']['reg_form'] == 'on') {
				...
				add_action('user_register', function ($userId){
					if ($_POST['fm-sign'] == 'on') {
						...
						$this->addNewEmail(array('email' => $user->user_email), $_POST['fm-sign-referer'], __('Registration Form', 'wp_freshmail'), $this->fmSettings['fm_sign_up_checkboxes']['freshmail_list_id']);
					}
				}, 90, 1);
			}
		}
		...
		if (isset($this->fmSettings['fm_sign_up_checkboxes']['sign_up_checkboxes']['woo_form'])) {
			if ($this->fmSettings['fm_sign_up_checkboxes']['sign_up_checkboxes']['woo_form'] == 'on') {
				...
				add_action('woocommerce_checkout_process', function (){
					if ($_POST['fm-sign'] == 'on') {
						$this->addNewEmail(array('email' => $_POST['billing_email']), $_POST['fm-sign-referer'], __('WooCommerce Checkout', 'wp_freshmail'), $this->fmSettings['fm_sign_up_checkboxes']['freshmail_list_id']);
					}
				});
			}
		}
		...
		if (isset($this->fmSettings['fm_sign_up_checkboxes']['sign_up_checkboxes']['easy_dig_form'])) {
			if ($this->fmSettings['fm_sign_up_checkboxes']['sign_up_checkboxes']['easy_dig_form'] == 'on') {
				...
				add_action('edd_purchase', function (){
					if ($_POST['fm-sign'] == 'on') {
						$this->addNewEmail(array('email' => $_POST['edd_email']), $_POST['fm-sign-referer'], __('Easy Digital Download Checkout ', 'wp_freshmail'), $this->fmSettings['fm_sign_up_checkboxes']['freshmail_list_id']);
					}
				}, 9);
			}
		}
		...
		add_action('wpcf7_init', function (){
			...
			add_action('wpcf7_posted_data', function ($data = array()){
				...
				if (isset($data['fm-sign']) && $data['fm-sign'] == 'on' && $email != null) {
					$this->addNewEmail(array('email' => $email), $data['fm-sign-referer'], __('Contact form 7', 'wp_freshmail'), $this->fmSettings['fm_sign_up_checkboxes']['freshmail_list_id']);
				}
