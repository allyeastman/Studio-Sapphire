			...
			<form method="post" class="form_subscribe freshmail_form_<?php echo $fmFormId['id']; ?>">
				...
				if (isset($freshmailForm['fields']) && is_array($freshmailForm['fields'])):
					foreach ($freshmailForm['fields'] as $key => $val):
						if (is_array($val)):
							if (!isset($val['include']) || $val['include'] == 'on'):
							...
							<div>
								<label style="<?php echo $label['position'].';width:'.$label['width'].$label['select_width'].';color:'.$label['text_color'].';font-size:'.$label['text_size'].'pt; '.$label['select_text_style']; ?>" class="label">
									<?php echo $val['value'];
									echo (!isset($val['required']) || $val['required'] == 'on' ? '<span style="color:red;">*</span>' : null); ?>
								</label>
							</div>
							<?php endif; ?>
								<div style="<?php echo 'margin: '.$field['vertical_margin'].$field['vertical_margin_type'].' '.$field['horizontal_margin'].$field['horizontal_margin_type'].';'; ?>">
									<input type="text" class="field" placeholder="<?php echo (isset($label['display_placeholders']) && $label['display_placeholders'] == 'yes') ? (isset($val['placeholder']) ? $val['placeholder'] : $val['value']) : null; ?>" value="" name="form[<?php echo $key; ?>]" style="<?php echo 'max-width:100%;width:'.$field['width'].$field['select_field_width'].'; height:'.$field['height'].$field['select_field_height'].';color:'.$field['text_color'].';font-size:'.$field['text_size'].'pt; border:'.$field['border_width'].'px solid '.$field['border_color'].';';?>" />
								</div>
							<?php	endif;
						endif;
					endforeach;
				...
				if ($checkboxAgreement['display'] == 'yes'): ?>
					<p style="<?php echo 'color:'.$checkboxAgreement['text_color'].';font-size:'.$checkboxAgreement['text_size'].'pt;text-align:'.$checkboxAgreement['text_aligment'].';'; ?>">
