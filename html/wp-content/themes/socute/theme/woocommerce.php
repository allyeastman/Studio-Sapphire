...
function yit_add_featured_products_slider_image_size( $options ) {
    ...
    $field_featured = array(
        'name' => __( 'Featured Products Widget', 'yit' ),
        'desc' 		=> sprintf(__('%1$sThis size is usually used for the products thubmnails in the slider widget Featured Products.%2$s', 'yit' ),'<p><small>','</small></p>'),
    ...
    if ( version_compare( preg_replace( '/-beta-([0-9]+)/', '', $woocommerce->version ), '2.1', '<' ) ) {
        ...
        $options[] = $field_featured;

        $options[] = $field_responsive;

        array_push( $options, $tmp );
    }
    elseif ( version_compare( preg_replace( '/-beta-([0-9]+)/', '', $woocommerce->version ), '2.3', '<' ) ) {
        $offset  = - 6;
        ...
        //unset temporary the end section
        $end_section = $options[13];
        unset( $options[13] );

        $options = array_merge( $options, array( $field_featured, $field_responsive ) );

        //readd the end section after my new options
        $options[] = $end_section;
...
function yit_get_featured_image_size() {
    ...
    $element = get_option( 'shop_featured_image_size', array() );
